<template>
  <previewWrapper :pageId="pageId" @closePreview="closePreview" v-loading="loading">
    <p class="page-title paddingL30 white">页面预览</p>
    <div class="preview-info-wrapper" v-if="!loading">
      <el-form label-width="90px" label-position="left">
        ...
        <el-form-item label="页面链接:">
          ...
          <div class="share-wx-config-wrapper">{{$config.baseURL + '/c317/view/' + pageId}}</div>
        </el-form-item>
        <!--页面效果-->
...
<script>
    ...
    computed: {
			pageLink(){
				return this.$config.baseURL + '/c317/view/' + this.pageId
      },
			shareData(){
